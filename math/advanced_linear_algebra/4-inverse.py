...
This module provides a single function `inverse(matrix)` that
validates input and computes the inverse matrix.
"""
...
def adjugate(matrix):
    """Helper function to compute adjugate (transpose of cofactor matrix)."""
    n = len(matrix)
    if n == 1:
        return [[1]]
...
def inverse(matrix):
    ...
    Returns:
        list of lists: inverse matrix, or None if singular
